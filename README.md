...
- 🔍 **Job-Discovery** - Automatische Stellensuche aus RSS-Feeds + Bundesagentur/Adzuna/Arbeitnow APIs mit KI-Match-Score gegen deinen CV (Provider frei wählbar). Inkl. Quellen-Verwaltung, Pagination, Volltextsuche, Filter (Remote/PLZ/Anstellungsart), Firmen-Reject-Filter, Browser-Push für Top-Matches und Onboarding-Checkliste
...
- 🔍 **Job-Discovery** - Automated job search from RSS feeds + Bundesagentur/Adzuna/Arbeitnow APIs with AI match-scoring against your CV (provider freely selectable). Includes source management, pagination, full-text search, filters (Remote/postal-code/employment-type), rejected-company filtering, browser push notifications for top matches, and onboarding checklist
