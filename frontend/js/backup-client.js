 ...
 * Fixed to properly handle Auth.fetch() which returns parsed JSON (not Response objects)
 */
...
class BackupClient {
    ...
    async listBackups() {
        try {
            ...
            // Auth.fetch returns parsed JSON directly, not a Response object
            return data.backups || [];
        } catch (error) {
    ...
    async getBackup(version, decrypt = true) {
        try {
            const url = `${this.baseUrl}/${version}?decrypt=${decrypt}`;
            const data = await Auth.fetch(url, {
                method: 'GET'
            });
            // Auth.fetch returns parsed JSON directly
            return data;
        } catch (error) {
    ...
    async restoreBackup(version, clearExisting = false) {
        try {
            const data = await Auth.fetch(`${this.baseUrl}/${version}/restore`, {
                method: 'POST',
                body: JSON.stringify({
                    confirm: true,
...
// Create global instance if we're not in a module environment
